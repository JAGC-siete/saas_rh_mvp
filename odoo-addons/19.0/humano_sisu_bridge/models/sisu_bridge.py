...
class HumanoSisuBridge(models.TransientModel):
    ...
    def _write_identification(self, employee, identification_id):
        """Odoo 19: identification_id lives on hr.version; _inherits delegates writes."""
        if identification_id is None:
            return
        employee.identification_id = identification_id
        version = employee.version_id or employee.current_version_id
        if version:
            version.identification_id = identification_id
    ...
    def upsert_employee(self, vals):
        ...
        self._write_identification(employee, vals.get('identification_id'))
