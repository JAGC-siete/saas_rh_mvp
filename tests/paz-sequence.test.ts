...
describe('paz → info sequence (video pack)', () => {
  ...
  it('pack variant paz includes video + unlock links and Paper Bridge body', () => {
    ...
    assert.ok(pack.includes('Resumen del método'))
    assert.ok(pack.includes('Registrar asistencia con biométrico conectado al software'))
  ...
  it('html pack variant paz includes watch and unlock hrefs', () => {
    ...
    assert.ok(html.includes('Resumen del método'))
    assert.ok(html.includes('sin USB ni planillas sueltas'))
  })
})
