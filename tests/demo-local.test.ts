...
describe('demo-local landing', () => {
  ...
  it('vende transformación (clientes y tiempo) y no un inventario de fierros', () => {
    ...
    assert.equal(DEMO_LOCAL_COPY.hero.ctaPrimary, 'Ver cómo funcionaría para mi negocio')
    assert.equal(DEMO_LOCAL_COPY.form.submit, 'Solicitar mi propuesta gratuita')
    ...
    assert.match(DEMO_LOCAL_COPY.problem.items[0].body, /competencia/)
    assert.match(DEMO_LOCAL_COPY.problem.items[1].title, /WhatsApp/)
