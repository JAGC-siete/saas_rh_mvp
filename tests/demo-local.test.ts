...
describe('demo-local landing', () => {
  ...
  it('vende servicio (qué, cómo, cuánto) y no se presenta como planilla', () => {
    ...
    assert.equal(DEMO_LOCAL_COPY.hero.headline, 'Tu comercio local en Internet hoy')
    assert.equal(DEMO_LOCAL_COPY.hero.subheadline, 'Página web, Sistema de reservas, y Whatsapp')
    assert.equal(blob.includes('Servicio de presencia digital · negocios de barrio'), false)
    ...
    assert.equal(DEMO_LOCAL_COPY.offer.title, 'Cómo lo hacemos')
    assert.equal(DEMO_LOCAL_COPY.offer.steps[1].title, 'Aparece en Google Maps')
