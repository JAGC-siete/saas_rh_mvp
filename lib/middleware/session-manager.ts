...
import { extractSessionIdFromSession } from '../auth/session-revoke'

/**
 * Extract Auth session id from session (session_id claim, then jti, then hash fallback)
 */
function extractJtiFromSession(session: any): string | null {
  try {
    return extractSessionIdFromSession(session)
  } catch (error) {
    logger.error('Failed to extract session id from session', {
      error,
