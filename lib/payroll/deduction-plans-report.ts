  ...
  return new Promise<Buffer>((resolve, reject) => {
    try {
      ...
      let yPos = drawLiquidPdfHeader(doc, {
        title: 'Reporte de Planes de Deducción',
        subtitle: `Generado: ${formatDateForHonduras(nowInHonduras())}`,
        tagline: companyName || 'HUMANO SISU',
        height: 90,
      })

      doc.fillColor(PDF.bodyText)
      doc.fontSize(11).text('INFORMACIÓN DEL REPORTE:', 30, yPos)
      yPos += 16
      ...
      if (generatedByEmail) {
        yPos += 16
        doc.fontSize(10).text(`Generado por: ${generatedByEmail}`, 30, yPos)
      }
      ...
      doc.rect(30, yPos, pageWidth - 60, 90).stroke(PDF.panelBorder)
      doc.fontSize(12).text('RESUMEN EJECUTIVO', 40, yPos + 10)
      doc.fontSize(9).text('Total planes:', 45, yPos + 30)
      doc.fontSize(9).text(String(totalPlans), 200, yPos + 30)
      doc.fontSize(9).text('Activos:', 45, yPos + 46)
      doc.fontSize(9).text(String(activos), 200, yPos + 46)
      doc.fontSize(9).text('Inactivos:', 45, yPos + 62)
      doc.fontSize(9).text(String(inactivos), 200, yPos + 62)
      doc.fontSize(9).text('Empleados con planes:', 45, yPos + 78)
      doc.fontSize(9).text(String(empleadosUnicos), 200, yPos + 78)
      doc.fontSize(9).text('Monto total (planes activos):', 360, yPos + 30)
      doc.fontSize(9).text(formatCurrency(totalMonto), 520, yPos + 30)

      yPos += 100

      if (plans.length === 0) {
        doc.fontSize(10).text('No hay planes de deducción registrados.', 30, yPos)
        doc.end()
        return
      }
      ...
      doc.rect(x, y, colWidths[i], rowHeight).fillAndStroke(PDF.tableHeader, PDF.bodyText)
        doc.fillColor('white')
