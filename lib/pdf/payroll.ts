...
export async function generateGeneralPayrollPDFBuffer(payrollRecords: any[], periodo: string): Promise<Buffer> {
	...
	return await new Promise<Buffer>((resolve, reject) => {
		...
		const headers = ['Código', 'Nombre', 'Departamento', 'Días Trabajados', 'Salario Bruto', 'Deducciones', 'Salario Neto', 'Estado']
		...
		payrollRecords.forEach((record: any) => {
			const values = [
				...
				record.days_worked?.toString?.() || '0',
				`L. ${Number(record.gross_salary || 0).toFixed(2)}`,
				`L. ${Number(record.total_deductions || 0).toFixed(2)}`,
				`L. ${Number(record.net_salary || 0).toFixed(2)}`,
				record.status || '',
			]
			values.forEach((val, i) => {
				const x = startX + colWidths.slice(0, i).reduce((a, b) => a + b, 0)
				doc.rect(x, y, colWidths[i], rowHeight).stroke()
				doc.text(String(val), x + 2, y + 3, { width: colWidths[i] - 4 })
			})
			y += rowHeight
		})

		const totalBruto = payrollRecords.reduce((sum: number, r: any) => sum + (Number(r.gross_salary) || 0), 0)
		const totalDeducciones = payrollRecords.reduce((sum: number, r: any) => sum + (Number(r.total_deductions) || 0), 0)
		const totalNeto = payrollRecords.reduce((sum: number, r: any) => sum + (Number(r.net_salary) || 0), 0)
...
export async function generateEmployeeVoucherPDFBuffer(payrollRecord: any, periodo: string, quincena: number): Promise<Buffer> {
	const PDFDocument = require('pdfkit')
	...
	return await new Promise<Buffer>((resolve, reject) => {
		...
		doc.fontSize(10).text(String(payrollRecord.days_worked ?? 0), 380, 155)
		...
		doc.fontSize(10).text(`L. ${Number(payrollRecord.base_salary || 0).toFixed(2)}`, 400, 240)
		...
		doc.fontSize(10).text(`L. ${Number(payrollRecord.social_security || 0).toFixed(2)}`, 400, 320)
		doc.fontSize(10).text('RAP (1.5%):', 40, 335)
		doc.fontSize(10).text(`L. ${Number(payrollRecord.professional_tax || 0).toFixed(2)}`, 400, 335)
		doc.fontSize(10).text('ISR (Impuesto Sobre la Renta):', 40, 350)
		doc.fontSize(10).text(`L. ${Number(payrollRecord.income_tax || 0).toFixed(2)}`, 400, 350)
		doc.fontSize(10).text('Total Deducciones:', 40, 365)
		doc.fontSize(10).text(`L. ${Number(payrollRecord.total_deductions || 0).toFixed(2)}`, 400, 365)
		...
		doc.fontSize(14).text(`L. ${Number(payrollRecord.net_salary || 0).toFixed(2)}`, 400, 425, { align: 'right' })
		...
		doc.fontSize(10).text(`L. ${Number(payrollRecord.net_salary || 0).toFixed(2)}`, 400, 505)
