...
export type VentasAcceleratedPainPointsResult = {
  eligible: number
  sent: number
  dryRun?: boolean
  error?: string
...
async function hasPainPointInLedger(
  ...
  if (error) {
    logger.warn('Ventas accelerated sequence: ledger lookup failed', {
      leadId,
      step,
      error: error.message,
    })
...
async function recordLedgerEntry(supabase: SupabaseClient, leadId: string, step: number) {
  ...
  const { error } = await supabase.from('marketing_email_ledger').insert({
    ...
    subject: getVentasSequenceSubject(step),
    watch_window_key: `ventas-accel:${step}`,
  })

  if (error) {
    logger.warn('Ventas accelerated sequence: failed to record ledger', {
      leadId,
      step,
      error: error.message,
    })
...
/** Sends PP1–PP5 to /ventas leads on a fixed 48h cadence. */
export async function runVentasAcceleratedPainPoints(
  now: Date = new Date()
): Promise<VentasAcceleratedPainPointsResult> {
  ...
  if (error) {
    logger.error('Ventas accelerated sequence: fetch failed', { error: error.message })
  ...
  for (const lead of ventasLeads) {
    ...
    try {
      ...
      sent += 1
      logger.info('Ventas accelerated pain point sent', {
        email: lead.email,
        step,
        label: content.label,
        dryRun,
      ...
      logger.error('Ventas accelerated pain point failed', {
        email: lead.email,
        step,
        error: message,
      })
