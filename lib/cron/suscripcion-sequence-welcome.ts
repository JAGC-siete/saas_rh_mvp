...
import { getSuscripcionSequenceSubject } from '../marketing/suscripcion-field-notes-email'
import { SUSCRIPCION_SEQUENCE_WELCOME_DELAY_HOURS } from '../marketing/suscripcion-sequence-timing'
...
export type SuscripcionSequenceWelcomeResult = {
  eligible: number
  sent: number
  dryRun?: boolean
  error?: string
...
async function hasWelcomeInLedger(leadId: string): Promise<boolean> {
  ...
  if (error) {
    logger.warn('Suscripcion welcome scheduler: ledger lookup failed', { leadId, error: error.message })
    return true
  }
...
/** Sends Step 0 welcome to /suscripcion leads 24h after the recibo pack. */
export async function runSuscripcionDelayedSequenceWelcome(
  now: Date = new Date()
): Promise<SuscripcionSequenceWelcomeResult> {
  const dryRun = process.env.WATCHMAN_DRY_RUN === 'true'
  const cutoff = new Date(now.getTime() - SUSCRIPCION_SEQUENCE_WELCOME_DELAY_HOURS * 60 * 60 * 1000)
  ...
  if (error) {
    logger.error('Suscripcion welcome scheduler: fetch failed', { error: error.message })
    return { eligible: 0, sent: 0, error: error.message, dryRun }
  }

  const suscripcionLeads = (leads ?? []).filter(
    (lead) => normalizeLeadSource(lead.source) === 'suscripcion' && lead.unsubscribe_token
  )
    ...
    try {
      ...
      if (!dryRun) {
        await supabaseAdmin.from('marketing_email_ledger').insert({
          ...
          subject: getSuscripcionSequenceSubject(0),
      ...
      sent += 1
      logger.info('Suscripcion delayed sequence welcome sent', { email: lead.email, dryRun })
    } catch (e: unknown) {
      const message = e instanceof Error ? e.message : 'Unknown error'
      logger.error('Suscripcion delayed sequence welcome failed', { email: lead.email, error: message })
    }
  }
