// 🔧 TEST ALL PAYROLL ENDPOINTS - Verificar todos los endpoints del sistema
// Ejecutar: node test-all-payroll-endpoints.js
...
async function runAllTests() {
  log('🚀 INICIANDO PRUEBAS COMPLETAS DEL SISTEMA DE NÓMINA', 'bold');
  log('=' .repeat(60), 'blue');
  ...
  // ========================================
  // 2. ENDPOINTS DE ASISTENCIA
  // ========================================
  log('\n📊 SECCIÓN 2: ENDPOINTS DE ASISTENCIA', 'bold');

  // Lookup de empleados
  result = await testEndpoint('Lookup Empleados', 'GET', '/api/attendance/lookup?dni=0801-1990-12345');
  results.total++; results[result.success ? 'passed' : 'failed']++;
  results.details.push({ name: 'Lookup Empleados', ...result });
  ...
  // Debug de asistencia
  result = await testEndpoint('Debug Asistencia', 'GET', '/api/attendance/debug');
  results.total++; results[result.success ? 'passed' : 'failed']++;
  results.details.push({ name: 'Debug Asistencia', ...result });
  ...
  results.total++; results[result.success ? 'passed' : 'failed']++;
  results.details.push({ name: 'Login', ...result });
