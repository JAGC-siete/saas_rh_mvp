...
module "eks" {
  ...
  cluster_name    = "${var.project_name}-${var.environment}-eks2"
...
module "monitoring" {
  ...
  # Using the -v2 resources
  eks_cluster_name       = "${var.project_name}-${var.environment}-eks2"
  rds_instance_id        = "${var.project_name}-db-${var.environment}-v2"
  elasticache_cluster_id = "${var.project_name}-redis-${var.environment}-v2"
  tags = {
    Environment = var.environment
    Team        = var.team_tag
    CostCenter  = var.cost_center_tag
...
# Temporarily commented out until EKS outputs are properly configured
