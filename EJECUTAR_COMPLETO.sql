...
INSERT INTO companies (id, name, subdomain, settings, status, created_at, updated_at) 
VALUES (
...
-- Verificar empleados con horario asignado (muestra de 10)
SELECT 
    e.name,
...
-- Detectar duplicados en últimos 5 dígitos DNI
SELECT 
...
SELECT 
    ...
    COUNT(*) as total_empleados,
    COUNT(CASE WHEN work_schedule_id IS NOT NULL THEN 1 END) as empleados_con_horario,
    COUNT(CASE WHEN status = 'active' AND work_schedule_id IS NOT NULL THEN 1 END) as activos_listos_para_asistencia
FROM employees;
