-- Integral fix: "column c.timezone does not exist" on calculate_attendance_hours_batch.
-- Cause: DB without migration 20260412120000_multipay_fundamentals (no companies.timezone).
-- 1) Ensure companies.country_code + companies.timezone exist (idempotent).
-- 2) Backfill from settings->>'timezone' (admin onboarding stores it there) then by country_code.
-- 3) Batch function: resolve IANA as COALESCE(column, settings JSON, country default).

ALTER TABLE public.companies ADD COLUMN IF NOT EXISTS country_code TEXT;

UPDATE public.companies
SET country_code = 'HND'
...
COMMENT ON COLUMN public.companies.timezone IS
  'IANA timezone. App may also store settings.timezone; COALESCE in batch RPC.';

UPDATE public.companies c
SET timezone = NULLIF(TRIM(COALESCE(c.settings->>'timezone', '')), '')
WHERE (c.timezone IS NULL OR TRIM(COALESCE(c.timezone, '')) = '')
  AND NULLIF(TRIM(COALESCE(c.settings->>'timezone', '')), '') IS NOT NULL;

UPDATE public.companies c
SET timezone = CASE COALESCE(c.country_code, 'HND')
...
COMMENT ON FUNCTION public.calculate_attendance_hours_batch(UUID[], INTEGER) IS
  'Batch hours: TZ = COALESCE(companies.timezone, settings.timezone JSON), else country default. ON CONFLICT uses constraint name (no RETURNS TABLE shadowing).';
