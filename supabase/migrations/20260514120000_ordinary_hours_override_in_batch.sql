-- Optional company cap for ordinary daily hours before overtime (company_payroll_configs.metadata.ordinary_hours_override).
-- COALESCE(override, labor_laws.legal_daily_hours, 8).
...
COMMENT ON FUNCTION public.calculate_attendance_hours_batch(UUID[], INTEGER) IS
  'Batch hours: ordinary cap = COALESCE(company_payroll_configs.metadata.ordinary_hours_override, labor_laws.legal_daily_hours, 8). TZ from companies. ON CONFLICT attendance_record_id.';
