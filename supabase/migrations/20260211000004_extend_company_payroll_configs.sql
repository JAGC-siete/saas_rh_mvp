...
ALTER TABLE company_payroll_configs ADD COLUMN IF NOT EXISTS
  payment_frequency TEXT CHECK (payment_frequency IS NULL OR payment_frequency IN ('quincenal', 'mensual', 'semanal'));
...
COMMENT ON COLUMN company_payroll_configs.payment_frequency IS 'Default payment frequency for company (quincenal, mensual, semanal). Overridden by employees.payment_frequency.';
