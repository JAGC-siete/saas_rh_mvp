-- Use shift_config topology when inferring break minutes in batch hours calculation.
...
COMMENT ON FUNCTION public.calculate_attendance_hours_batch(UUID[], INTEGER) IS
  'Batch hours with shift_config-aware break inference (split gap vs continuous break).';
