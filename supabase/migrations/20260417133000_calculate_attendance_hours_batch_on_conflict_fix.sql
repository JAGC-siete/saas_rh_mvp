-- Fix: "column reference attendance_record_id is ambiguous" when calling
-- calculate_attendance_hours_batch from API (RPC). In PL/pgSQL, RETURNS TABLE
-- output names are variables and shadow table columns inside ON CONFLICT (...).
-- Use explicit unique constraint name so the conflict target is not ambiguous.
...
COMMENT ON FUNCTION public.calculate_attendance_hours_batch(UUID[], INTEGER) IS
  'Batch hours: labor_laws por país; IANA desde companies.timezone o fallback por country_code. ON CONFLICT uses constraint name to avoid RETURNS TABLE name shadowing.';
