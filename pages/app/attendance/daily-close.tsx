...
export default function AttendanceDailyClosePage() {
  ...
  const finalizeClose = async () => {
    if (!confirm('Finalizar cierre: marcar registros del día como cerrados y recalcular horas (requiere entrada y salida donde aplique). ¿Continuar?')) {
      return
    }
  ...
  const openEdit = (item: DailyCloseItem) => {
    const rec = item.record as { check_in?: string; check_out?: string; lunch_start?: string; lunch_end?: string } | null
    setEditItem(item)
          ...
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <p className="text-gray-400 text-sm mt-1">
                Consolida marcas biométricas en registros del día, corrige anomalías y finaliza cuando corresponda.
                La finalización calcula horas solo si hay entrada y salida definidas.
              </p>
            </div>
