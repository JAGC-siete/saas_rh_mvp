  ...
  try {
    ...
    const processedEmployees = employees?.map((emp: any) => {
      ...
      return {
        ...
        check_in_time,
        check_out_time,
        work_schedule: emp.work_schedules?.[0] ? {
          name: emp.work_schedules[0].name,
          start_time: emp.work_schedules[0].monday_start,
          end_time: emp.work_schedules[0].monday_end
        } : null,
        gamification: emp.employee_scores?.[0] ? {
          total_points: emp.employee_scores[0].total_points || 0,
          weekly_points: emp.employee_scores[0].weekly_points || 0,
          monthly_points: emp.employee_scores[0].monthly_points || 0,
          achievements_count: 0 // TODO: Add achievements count
        } : {
          total_points: 0,
          weekly_points: 0,
          monthly_points: 0,
    ...
    console.error('Error in employee search API:', error)
