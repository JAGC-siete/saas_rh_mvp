...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc('reports_payroll_summary', {
    ...
    if (error) {
      logger.error('Error calling reports_payroll_summary RPC', { error, companyId })
    ...
    logger.error('Reports payroll summary API error', { error, message: error.message })
